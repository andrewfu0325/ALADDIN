...
/*  char *in_file;
  ...
  TYPE *cube = (TYPE*)(malloc)(cube_height_size*cube_row_size*cube_col_size*sizeof(TYPE)*3);
  struct spad_t spad;
  spad.C[0] = 100;
  spad.C[1] = 200;
  spad.C[2] = 100;
...
// This is used for trace address to sim-virtual address translation
  ...
  for(int h = 0; h < cube_height_size; h++) 
        ...
        for(int it = 0; it < 3; it++) 
				  cube[((h * cube_row_size + r) * cube_col_size + c) * 3 + it] = h + r + c + it;
  ...
  for(int h = 0; h < height_size; h++) {
		for(int r = 0; r < row_size; r++) {
			for(int c = 0; c < col_size; c++) {
				args[it - 1].orig[(h * row_size + r) * col_size + c] = 
          (cube[off + ((h * cube_row_size + r) * cube_col_size + c) * 3 + 0] +
          cube[off + ((h * cube_row_size + r) * cube_col_size + c) * 3 + 1] +
          cube[off + ((h * cube_row_size + r) * cube_col_size + c) * 3 + 2]) / 3;
			}
		}
  }
	enable[it] = 1;
