...
// This is used for trace address to sim-virtual address translation
  ...
  m5_work_begin(0,0);
	...
	for(int j = 0; j < row_size; j++) {
      ...
      int c, e, s, se;
      c = image[j * 2 * image_col_size + i * 2];
      if(i * 2 == image_col_size - 1) 
        e = c;
      else
        e = image[j * 2 * image_col_size + i * 2 + 1];
      if(j * 2 == image_row_size - 1)
        s = c;
      else
        s = image[(j * 2 + 1) * image_col_size + i * 2];
      if(i * 2 == image_col_size - 1 || j * 2 == image_row_size - 1)
        se = c;
      else
        se = image[(j * 2 + 1) * image_col_size + i * 2 + 1];
      args[it-1].orig[j * col_size + i] = (c + e + s + se) / 4;
		}
	}
	printf("DONE %d\n", it - 1);
	enable[it] = 1;
