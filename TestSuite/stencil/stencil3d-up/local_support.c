  ...
  for(int h = 0; h < height_size; h++) {
		for(int r = 0; r < row_size; r++) {
			for(int c = 0; c < col_size; c++) {
				int c, e, s, se;
				c = cube[(h * cube_row_size + r * 2) * cube_col_size + c * 2];
				if(c * 2 == cube_col_size - 1)
					e = c;
				else
					e = cube[(h * cube_row_size + r * 2) * cube_col_size + c * 2 + 1];
				if(r * 2 == cube_row_size - 1)
					s = c;
				else
					s = cube[(h * cube_row_size + (r * 2 + 1)) * cube_col_size + c * 2];
				if(c * 2 == cube_col_size - 1 || r * 2 == cube_row_size - 1)
					se = c;
				else
					se = cube[(h * cube_row_size + (r * 2 + 1)) * cube_col_size + c * 2 + 1];
				args[it-1].orig[(h * row_size + r) * col_size + c] = (c + e + s + se) / 4;
			}
		}
  }
	enable[it] = 1;
