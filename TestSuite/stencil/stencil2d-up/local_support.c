...
// This is used for trace address to sim-virtual address translation
  ...
  m5_work_begin(0,0);
	...
	for(int j = 0; j < row_size; j++) {
      ...
      int tmp;
      if(i == col_size - 1 || j == row_size - 1 || (i % 2 == 0 && j % 2 == 0))
          tmp = image[(j / 2) * image_col_size + i / 2];
      else if(i % 2 == 0 && j % 2 == 1)
        tmp = (image[(j / 2) * image_col_size + i / 2] +
              image[(j / 2 + 1) * image_col_size + i / 2]) / 2;
      else if(i % 2 == 1 && j % 2 == 0) 
        tmp = (image[(j / 2) * image_col_size + i / 2] +
              image[(j / 2) * image_col_size + i / 2 + 1]) / 2;
      else if(i % 2 == 1 && j % 2 == 1)
        tmp = (image[(j / 2) * image_col_size + i / 2] +
              image[(j / 2 + 1) * image_col_size + i / 2] +
              image[(j / 2) * image_col_size + i / 2] +
              image[(j / 2) * image_col_size + i / 2 + 1]) / 4;
      args[it - 1].orig[j * col_size + i] = tmp;
    }
	}
	printf("DONE %d\n", it - 1);
	enable[it] = 1;
