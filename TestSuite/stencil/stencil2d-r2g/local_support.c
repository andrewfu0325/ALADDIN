...
/*  char *in_file;
  ...
  TYPE *image = (TYPE*)(malloc)(image_row_size*image_col_size*sizeof(TYPE)*3);
  struct spad_t spad;
...
// This is used for trace address to sim-virtual address translation
  ...
  m5_work_begin(0,0);
	...
	for(int j = 0; j < row_size; j++) {
      ...
      //int accu = 0;
      for(int k = 0; k < 3; k++){
        args[it-1].orig[j * col_size + i] += (image[(j * image_col_size + i) * 3 + k]);
      }
      args[it-1].orig[j * col_size + i] /= 3;
		}
	}
	printf("DONE %d\n", it - 1);
	enable[it] = 1;
