...
// This is used for trace address to sim-virtual address translation
  ...
  mapArrayToAccelerator(
      MACHSUITE_STENCIL_3D, "C", (void*)&args[0].C, sizeof(struct bench_args_t) * NUM_ACC_TASK);
  mapArrayToAccelerator(
  ...
  int it = 1;
	...
	regAccTaskDataForCache(MACHSUITE_STENCIL_3D, &args[it - 1], sizeof(args[0].orig) + sizeof(args[0].C));
  #endif
#endif
