...
// This is used for trace address to sim-virtual address translation
  ...
  for(int r = 0; r < row_size; r++) {
		for(int c = 0; c < col_size; c+=(col_size/2)) {
	    for(int h = 0; h < height_size; h+=(height_size/4)) {
			  for(int cc = c; cc < c+(col_size/2); cc++) {
				  for(int hh = h; hh < h+(height_size/4); hh++) {
						args[it - 1].orig[(cc * row_size + r) * col_size + hh] = 
							cube[off + (hh * cube_row_size + r) * cube_col_size + cc];
					}
				}
			}
		}
