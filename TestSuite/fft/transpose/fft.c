...
void fft1D_512(TYPE work_x[FFT_SIZE*2], TYPE work_y[FFT_SIZE*2], struct bench_args_t *args, volatile int enable[NUM_ACC_TASK+1], volatile int avail[2]){
	...
	dmaLoad(work_x, args[0].work_x, 0, 0, WORK_SIZE, &enable[1], &avail[1]);
	dmaLoad(work_y, args[0].work_y, 0, 0, WORK_SIZE, &enable[1], NULL);
  if(NUM_ACC_TASK > 1) {
	  dmaLoad(work_x, args[0].work_x, WORK_SIZE, ACC_TASK_SIZE, WORK_SIZE, &enable[2], &avail[1]);
	  dmaLoad(work_y, args[0].work_y, WORK_SIZE, ACC_TASK_SIZE, WORK_SIZE, &enable[2], NULL);
  }
#endif
  ...
  for(int it = 0; it < NUM_ACC_TASK; it+=2) {
    loop1_buf1 : for(tid = 0; tid < THREADS; tid++){
					//GLOBAL_LOAD...
				...
				int sx, offset;
				sx = 66;
    loop2_buf1 : for(tid = 0; tid < 64; tid++){
					hi = tid>>3;
					lo = tid&7;
					offset = hi*8+lo;
				...
				sx = 8;
    loop3_buf1 : for(tid = 0; tid < 64; tid++){
					hi = tid>>3;
					lo = tid&7;
					offset = lo*66+hi;
				...
				sx = 66;
    loop4_buf1 : for(tid = 0; tid < 64; tid++){
					hi = tid>>3;
					lo = tid&7;
					offset = hi*8+lo;
    ...
    loop5_buf1 : for(tid = 0; tid < 64; tid++){
    ...
    loop6_buf1 : for(tid = 0; tid < 64; tid++){
				...
				//Transpose..
				sx = 72;
    loop7_buf1 : for(tid = 0; tid < 64; tid++){
					hi = tid>>3;
					lo = tid&7;
					offset = hi*8+lo;
					smem[0*sx+offset] = DATA_x[tid*8 + 0];
				...
				sx = 8;
    loop8_buf1 : for(tid = 0; tid < 64; tid++){
					hi = tid>>3;
					lo = tid&7;
					offset = hi*72+lo;
				...
				sx = 72;
    loop9_buf1 : for(tid = 0; tid < 64; tid++){
					hi = tid>>3;
					lo = tid&7;
					offset = hi*8+lo;
    ...
    loop10_buf1 : for(tid = 0; tid < 64; tid++){
					 data_y[0] = DATA_y[tid*8 + 0];
					 data_y[1] = DATA_y[tid*8 + 1];
					 data_y[2] = DATA_y[tid*8 + 2];
					 data_y[3] = DATA_y[tid*8 + 3];
    ...
    loop11_buf1 : for(tid = 0; tid < 64; tid++){
					 //Load post-trans
...
#ifdef DMA_INTERFACE_V4
    dmaStore(args[0].work_x, work_x, it*ACC_TASK_SIZE, 0, WORK_SIZE, NULL, &avail[1]);
    dmaStore(args[0].work_y, work_y, it*ACC_TASK_SIZE, 0, WORK_SIZE, NULL, NULL);
    if(it < NUM_ACC_TASK - 2) {
      dmaFence();
	    dmaLoad(work_x, args[0].work_x, 0, (it+2)*ACC_TASK_SIZE, WORK_SIZE, &enable[it+2+1], &avail[1]);
    	dmaLoad(work_y, args[0].work_y, 0, (it+2)*ACC_TASK_SIZE, WORK_SIZE, &enable[it+2+1], NULL);
    }
#endif
    if(NUM_ACC_TASK > 1) {
      loop1_buf2 : for(tid = 0; tid < THREADS; tid++){
					//GLOBAL_LOAD...
					data_x[0] = work_x[0*stride+tid];
					data_x[1] = work_x[1*stride+tid];
					data_x[2] = work_x[2*stride+tid];
				...
				int sx, offset;
				sx = 66;
      loop2_buf2 : for(tid = 0; tid < 64; tid++){
					hi = tid>>3;
					lo = tid&7;
					offset = hi*8+lo;
					smem[0*sx+offset] = DATA_x[tid*8 + 0];
				...
				sx = 8;
      loop3_buf2 : for(tid = 0; tid < 64; tid++){
					hi = tid>>3;
					lo = tid&7;
					offset = lo*66+hi;
				...
				sx = 66;
      loop4_buf2 : for(tid = 0; tid < 64; tid++){
					hi = tid>>3;
					lo = tid&7;
					offset = hi*8+lo;
      ...
      loop5_buf2 : for(tid = 0; tid < 64; tid++){
					data_y[0] = DATA_y[tid*8 + 0];
					data_y[1] = DATA_y[tid*8 + 1];
					data_y[2] = DATA_y[tid*8 + 2];
					data_y[3] = DATA_y[tid*8 + 3];
      ...
      loop6_buf2 : for(tid = 0; tid < 64; tid++){
					data_x[0] = DATA_x[tid*8 + 0];
					data_x[1] = DATA_x[tid*8 + 1];
					data_x[2] = DATA_x[tid*8 + 2];
					data_x[3] = DATA_x[tid*8 + 3];
				...
				//Transpose..
				sx = 72;
      loop7_buf2 : for(tid = 0; tid < 64; tid++){
					hi = tid>>3;
					lo = tid&7;
					offset = hi*8+lo;
					smem[0*sx+offset] = DATA_x[tid*8 + 0];
				...
				sx = 8;
      loop8_buf2 : for(tid = 0; tid < 64; tid++){
					hi = tid>>3;
					lo = tid&7;
					offset = hi*72+lo;
				...
				sx = 72;
      loop9_buf2 : for(tid = 0; tid < 64; tid++){
					hi = tid>>3;
					lo = tid&7;
					offset = hi*8+lo;
      ...
      loop10_buf2 : for(tid = 0; tid < 64; tid++){
					 data_y[0] = DATA_y[tid*8 + 0];
					 data_y[1] = DATA_y[tid*8 + 1];
					 data_y[2] = DATA_y[tid*8 + 2];
					 data_y[3] = DATA_y[tid*8 + 3];
      ...
      loop11_buf2 : for(tid = 0; tid < 64; tid++){
					 //Load post-trans
					 data_y[0] = DATA_y[tid*8];
					 data_y[1] = DATA_y[tid*8 + 1];
					 data_y[2] = DATA_y[tid*8 + 2];
...
#ifdef DMA_INTERFACE_V4
      dmaStore(args[0].work_x, work_x, (it+1)*ACC_TASK_SIZE, WORK_SIZE, WORK_SIZE, NULL, &avail[1]);
      dmaStore(args[0].work_y, work_y, (it+1)*ACC_TASK_SIZE, WORK_SIZE, WORK_SIZE, NULL, NULL);
      if(it < NUM_ACC_TASK - 2) {
        dmaFence();
	      dmaLoad(work_x, args[0].work_x, WORK_SIZE, (it+3)*ACC_TASK_SIZE, WORK_SIZE, &enable[it+3+1], &avail[1]);
      	dmaLoad(work_y, args[0].work_y, WORK_SIZE, (it+3)*ACC_TASK_SIZE, WORK_SIZE, &enable[it+3+1], NULL);
      }
#endif
    }
  }
